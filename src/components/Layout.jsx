  ...
  return (
    <aside className={`sidebar ${open ? "open" : ""}`}>
          ...
          return (
            <div key={group.section}>
              ...
              {visibleItems.map((item) => (
                <NavLink key={item.id} to={item.path} onClick={() => onClose()} className={({ isActive }) => `sb-item ${isActive ? "active" : ""}`}>
      ...
      <div className="sb-footer">
        <div className="sb-user" onClick={handleLogoutClick}>
          <div className="av">{initials(user?.fullName || user?.username)}</div>
...
export default function Layout({ user, onLogout, isAdmin, lowStockItems, lowStockCount, children }) {
  ...
  return (
    <div className="app-layout">
      ...
      <Sidebar user={user} onLogout={onLogout} open={open} onClose={() => setOpen(false)} isAdmin={isAdmin} lowStockCount={lowStockCount} />
